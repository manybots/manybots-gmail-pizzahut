...
class PizzahutWorker
  ...
  def items
    @items ||= self.manybots_client.activities({displayName: 'encomendas.pizzahut@ibersol.pt', verb: 'receive', object: 'email'}, 1, 100)['data']['items'] rescue(nil)
  end
  ...
  def self.perform(user_id)
    ...
    worker.items.each do |activity|
      meal = ManybotsGmailPizzahut::Meal.create_from_activity(user_id, activity)
